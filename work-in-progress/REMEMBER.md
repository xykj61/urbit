# REMEMBER — living operator card

*One-page carry card for outer terminal · phone · waymarks. Refreshed when Keaton says **remember**.*

**Last refreshed:** `20260724.151327` (EDT) · voice Quin  
**Product nib:** ZETA6 · LULU0–5 · **SETU6.5 · live-sent** · HOLD* · LIVE eyes · **HAWM folded**  
**Language nib:** **STOA0–332** · glow mode SLC GREEN · gate-spec shape_name GREEN  
**Suite nib:** ZETA0–6 · LULU0–5 · SETU0–SETU6.5 · live-sent · HOLD* · STOA0–332  
**Git nib:** `b82afda9a0` · flw-option-c

**Season:** Framework · EDT · **finishing edge:** newcomer clones · bootstraps · one witness green · one Glow line at `rishi glow` · inside thirty minutes.  
**APK:** `tools/.cache/tube05/sala-broadcaster.apk` · pkg `org.glow.app.sala_broadcaster`  
**ADB:** `tools/.cache/hawm0/android-sdk/platform-tools/adb` — not on bare PATH  

---

## Now

**flw Option C** `20260724.151327` — naming-grade fixture twin seated; waymark derive witness GREEN; genre law seated; runtime pre-flight clean; **vere/old STOP** (ops — five remotes / sibling clones on Framework host). Counsel reply [`../counsel/replies/20260724-151327_re-flw-corpus-shelf-option-c.md`](../counsel/replies/20260724-151327_re-flw-corpus-shelf-option-c.md).

**Witnesses:**

```bash
export RYE_ZIG=vendor/zig-toolchain/zig
rishi/bin/rishi run tools/waymark_derive_witness.rish
rishi/bin/rishi run tools/glow_repl_witness.rish
rishi/bin/rishi run tools/glow_jamcue_vectors_witness.rish
```

---

## Open doors

| Door | Kind |
|------|------|
| vere/old sibling-clone PR on five-remote host | check-in · pre-flight already green |
| Rishi list bound 256→512 (advise roster) | when convenient |
| Outward jam/cue publish | held · review docs/JAMCUE.md |
| Rung-1 digraph twin | after vere/old path clears |
| cast/face mold→shape | lean C held |

---

*Waymarks: HAWM · TUBE · ZETA · JABS · LULU · STOA · SETU · claims `waymarks/`*
