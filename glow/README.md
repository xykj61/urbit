...
**Language:** EN
**Last updated:** 2026-07-18 (**STOA23** two-face→add core · face→three · TAME + Acme first-code pointers)
**Style:** Radiant (see `../context/RADIANT_STYLE.md`)
**Voice:** Quin
**Audience:** Acme Corporation employees reading the working Glow pin
**Status:** Checkable — **STOA0–23** sixteen heads · two-face→add · face→three-arm · typed payload · library · multi-line · compose · nests · cells · `%` calls · truth, GREEN
...
All sixteen single-line heads lower today; multi-line adds compose (face→`|%` one to three arms · two faces→`add`), typed faces, and `|%` cores (up to three arms, bare or `@u32` payload, library emit). Desk files live under [`gen/`](gen/). Rishi hand: [`../tools/glow_run.rish`](../tools/glow_run.rish) · desk witness: [`../tools/glow_run_desk_witness.rish`](../tools/glow_run_desk_witness.rish). Brief: [`../active-designing/20260718-234122_stoa23-two-face-core-add-brief.md`](../active-designing/20260718-234122_stoa23-two-face-core-add-brief.md).

```bash
rishi/bin/rishi run tools/glow_run.rish glow/gen/compose-core-add.glow
rishi/bin/rishi run tools/glow_run_desk_witness.rish
# → GREEN: glow_run desk — all desk generators …
```

Each single-line hop lowers one named shape from its front-half parser. Under `|-`, `(lent …)` still emits a shrinking-list fold under `face.len`. Nest forms parse once through [`expr.rye`](expr.rye). Cell heads · `%` calls · multi-line bare/typed · face→call compose · face→`|%` (`compose-core` · `compose-core-two` · `compose-core-three` · `compose-core-add`) · `|%` cores (`core-double` · `core-two` · `core-three` · `core-payload` · `core-payload-typed`) · library emit (`lib-core-double`, no `main`) are GREEN. `=.` takes optional trailing decimals; `=*` takes an optional source wing. Cast hop covers `@u8` · `@u16` · `@u32` · `@u64`. `(cue …)`, nested jam, and face→payload compose stay **STOA24+**. Generator welcome/unwelcome paths use Zig/POSIX exits (0 / 1), per the Truth section above.
