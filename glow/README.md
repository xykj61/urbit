...
Each hop lowers one named shape from its front-half parser. Under `|-`, `(lent …)` still emits a shrinking-list fold under `face.len`. Nest forms parse once through [`expr.rye`](expr.rye): decimal · nested `^-  @uN  <decimal>` through `@u64` · bare face · **`(lent <face>)`** as a length payload under `=/` and as then/else or mint/send arms on `?:` · `?-` · `?~` (desk `face-lent` · `if-lent` · `switch-lent` · `null-lent`; witness [`../tools/glow_expr_witness.rish`](../tools/glow_expr_witness.rish)). `=.` takes optional trailing decimals; `=*` takes an optional source wing. Cast hop covers `@u8` · `@u16` · `@u32` · `@u64`. Other parenthesized values (`(mix …)`, …) and a fuller token stream stay later. Foundations and TAME stay the unmoved north star; this README is the working Glow surface pin. Generator welcome/unwelcome paths use Zig/POSIX exits (0 / 1), per the Truth section above.
