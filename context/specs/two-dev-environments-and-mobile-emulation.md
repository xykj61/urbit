...
**Language:** EN
**Stamp:** living ledger (born `20260715.193000`) · refreshed `20260715.230000` (M0-riscv rung added; frozen dated original now a pointer stub, per [`living-vs-dated.md`](living-vs-dated.md)) · refreshed `20260715.163500` (a third, interim macOS environment named; the Pixel/GrapheneOS ladder, G0–G3, opened separately from the Fairphone/postmarketOS M-ladder) · refreshed `20260716.115927` (the Pixel/GrapheneOS ladder's own G0–G3 renamed a waymark — a word drawn to fix a real collision with the unrelated Genode/seL4 proven-seat ladder's own G0/G1, per [`20260716-115927_waymark-ladder-naming-and-g0-collision-fix.md`](20260716-115927_waymark-ladder-naming-and-g0-collision-fix.md)) · corrected `20260716.124840` (the derivation now hashes with this tree's own SHA3-512, not plain SHA-2 — the word changed to **HAWM0–HAWM3**) · refreshed `20260716.131010` (HAWM0 setup landed on the Framework host: a checksum-verified portable JDK + Android cmdline-tools staged under `tools/.cache/hawm0/`, licenses accepted, platform-tools/emulator/API-36-google_apis-x86_64 system image installed, an AVD named `hawm0` created with a real Pixel-10-Pro-XL-shaped hardware profile; the emulator binary itself smoke-tested clean. Only the actual KVM boot remains, handed to a host-terminal script)
...
**The GrapheneOS/Pixel line needs its own ladder, not a reuse of M0–M3** (those name the Fairphone/postmarketOS body specifically). Named here, checked against the current state of the world (dated, cited, re-checkable as GrapheneOS's own build process changes). **Rungs renamed HAWM0–HAWM3** `20260716.115927`, hash family corrected `124840`: the bare letter "G" (for "Genode," this ladder's own first draft) collided with the wholly unrelated Genode/seL4 proven-seat ladder's own `G0`/`G1` (`docs/PROVEN_SEAT.md`) — both ladders independently reached for the same obvious first letter. `HAWM` is a waymark, drawn deterministically (SHA3-512 over `old/doc/spec/flw.txt`'s word corpus, this tree's own Canonical hash tier) rather than picked by hand, exactly so this class of collision cannot recur; full derivation and rationale in [`20260716-115927_waymark-ladder-naming-and-g0-collision-fix.md`](20260716-115927_waymark-ladder-naming-and-g0-collision-fix.md):

| Rung | Name | Where | State |
|---|---|---|---|
| **HAWM0** | **Stock AOSP/Android Emulator, HVF/KVM-accelerated** | This macOS host (blocked) · **Framework 16 AMD Ubuntu host (setup landed `20260716.131010`)** | **On macOS: blocked, not gated on missing tooling** — `20260715.163500`: a real, unrelated Homebrew ownership issue (`/opt/homebrew` owned by a different user than the current account, likely from an earlier `sudo brew install`) blocks installing `android-commandlinetools` via Homebrew, needing a one-time interactive `sudo chown -R $(whoami) /opt/homebrew` an agent session cannot supply. **On the Framework 16 AMD Ubuntu host: setup landed, boot handed off to a host-terminal script.** No Homebrew blocker (Linux-specific tooling instead); KVM-access **policy** decided by Keaton `20260716.124840` — host-terminal one-shot, matching `lane_kvm`'s already-GREEN Framework-metal precedent, no widening of counsel `090512`'s A-narrow boundary. From inside the jail (network-only work, no `/dev/kvm` needed for any of it): a checksum-verified portable Temurin 21 JDK and the Android command-line tools staged under `tools/.cache/hawm0/` (gitignored, real precedent already extends `tools/.cache/proven-seat/`'s own shape); all SDK licenses accepted; `platform-tools`, `emulator` (36.6.11), `platforms;android-36`, and `system-images;android-36;google_apis;x86_64` installed (5.5G total); one AVD named `hawm0` created and hand-tuned to a real Pixel 10 Pro XL hardware profile (1344×2992 resolution — the real, checked panel spec, 486 PPI — at a 560 logical-density bucket matching other real Pixel Pro-class AVD skins; 4G RAM — the flagship device this fork's own go-to-market brief names first) after `avdmanager --device` hit a real upstream bug (missing `devices.xml` at an unexpected path in cmdline-tools 22.0) worked around by creating with the generic profile and editing `config.ini` directly; the `emulator` binary itself smoke-tested clean (`-version` runs, all shared libraries resolve). **Only the actual KVM boot remains** — [`../../tools/hawm0_boot_onpath_host.sh`](../../tools/hawm0_boot_onpath_host.sh), run from a plain host terminal outside the jail (where `/dev/kvm` already exists on this host by the CPU/BIOS's own AMD-V, confirmed `20260716.110152`) |
| **HAWM1** | **Glow userland (Sala broadcaster) inside the AVD** | This macOS host | horizon — gate: HAWM0 · Sala B0/B1 (both already GREEN) |
| **HAWM2** | **Real GrapheneOS build, emulator target** | A Linux host (KVM) | horizon, and genuinely out of reach on macOS — GrapheneOS ships **no prebuilt emulator image** (checked directly, `20260715.163500`: `grapheneos.org/build`). Their own `sdk_phone64_x86_64` development-build target exists only by building the full GrapheneOS/AOSP source tree, and its own docs name **KVM** for hardware acceleration — Linux-only, unavailable on macOS's HVF. This rung needs a Linux host regardless of anything achievable on the Mac; the Framework 16 AMD Ubuntu host now on hand (`20260716`) is exactly this kind of host, KVM policy resolved at HAWM0 (same host-terminal path applies) |
| **HAWM3** | **Physical Pixel, GrapheneOS flashed** | Device | horizon — the real target device (Pixel 10 Pro XL flagship-first, per the go-to-market brief), gate: HAWM1. No physical Pixel owned yet as of `20260716` — Keaton is shopping used/new/refurbished; HAWM0–HAWM2 (all emulation) are the reachable work until one arrives |

**The honest register, named plainly rather than blurred:** HAWM0/HAWM1 exercise real AOSP/Android, the same userland surface GrapheneOS presents to an app, HVF/KVM-accelerated — genuinely useful for developing and testing Glow's Android-side code today. They are **not** GrapheneOS itself, and no claim here says otherwise; GrapheneOS's own hardening (Titan M2, MTE, verified boot) lives beneath the userland surface HAWM0/HAWM1 can reach, and only HAWM2's real build (on a Linux/KVM host) or HAWM3's physical device ever actually exercises it. This is the same "prove the compute, not the platform" honesty M0/M0-riscv already carry for the Fairphone/postmarketOS side.
...
- `20260716-115927_waymark-ladder-naming-and-g0-collision-fix.md` — why HAWM0–HAWM3 replaced G0–G3 here, and how to draw the next ladder's own mark.
- `../../docs/PROVEN_SEAT.md` — the unrelated Genode/seL4 ladder that kept bare G0/G1, the incumbent side of the same collision.
