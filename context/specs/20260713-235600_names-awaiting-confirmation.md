...
| **Kumara identity tiers** | Azimuth-style point/planet/star | **Decided** `20260714.044428` — Keaton **adopts** the Azimuth-style three-tier shape for Rhyz's own identity tiers (addressing + legible sponsor over the Ed25519 root; connectivity stays peer-to-peer). Resolves the long-open Kumara question. Exact tier *words* still open (a build detail) |

## Awaiting Confirm or Deny

Reya, Riyo, Trey, Triz (all four variants + their two-pair symmetry, `20260714.035600`), Maze, Neth, Ojjo, **Pool** (`20260714.042545`), **Rhyz** + the Azimuth-tier adoption (`20260714.044428`), the shell name (Rish/Rishi), and the module-vs-vane naming strategy are settled — see the Settled table above. What remains open:
...
| **Rhyz — tier words + build timing** | (name + Azimuth-tier shape confirmed) | The three-tier Azimuth-style shape is adopted; the exact tier *words* are open (a build detail). Seat Rhyz as a named horizon revival now (design carried, code deferred), or open its first Glow-native lap when ready? |
| **Quin — confirm + timing** | inference Q-vane (proposed) | Confirm Quin as the unifying vane of the four voices? Now-or-horizon like Pool. The sandbox loop = Quin × Puddle × Neth (compose, not a new vane) — accept, or name a Maze-nursery pattern? |
