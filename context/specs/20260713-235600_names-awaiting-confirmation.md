...
Every name proposed so far — Glow, Glow OS, Neth, Reya, Maze, Rish/Rishi, the module-vs-vane strategy, Ojjo (+ build timing), Riyo/Trey/Triz (+ the two-pair symmetry + redundancy shape), Pool (+ framing + timing), Rhyz (+ tier words + timing), Quin (+ the sandbox-loop composition), Sala (Realidream's revival-name) — is now settled. See the Settled table above.

**Nothing remains open on this page as of `20260715.175500`.** A future proposal (a new vane, a new tier word detail, a build-time refinement) adds its own row here when it arises; this page stays the one return point for "what have we named but not yet settled."
