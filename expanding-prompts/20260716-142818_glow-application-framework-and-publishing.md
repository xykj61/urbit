...
|---|---|---|---|
| **TUBE0** | **App manifest mold** | A `+$` mold for a Glow app manifest (name, version, declared capabilities, resource bounds), with a witness proving a well-formed manifest parses and a malformed one is refused at the mold boundary | Glow mold surface; no agent runtime needed — buildable as a pure value witness today, the smallest real first step |
