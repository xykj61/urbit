...
| **TUBE0.5** | **First installable Glow APK on GrapheneOS** *(named `20260717.014522`)* | A small, fixed, hand-written native-activity shim (never generated, reused verbatim across every Glow app) hands the window surface across a JNI seam to a Glow-built `.so`; the manifest mold's declared capabilities emit as `AndroidManifest.xml` permissions. The missing precondition beneath TUBE6/TUBE7: something installable to publish. Answers Keaton's own question about a Glow-to-Kotlin transpiler — named "no" in [`../active-designing/20260717-014522_native-activity-shim-and-slc-packaging.md`](../active-designing/20260717-014522_native-activity-shim-and-slc-packaging.md) | TUBE0 (GREEN) · HAWM3 (closed `20260717.013700`) — both gates closed; the rung itself stays horizon until the shim is written |
