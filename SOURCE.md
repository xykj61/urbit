...
**Language:** EN
**Version:** `20260714.081500` (Glow warm-aura date atom — chronological, later-is-larger)
**Style:** Radiant (see `context/RADIANT_STYLE.md`)
**By:** Rio 3, in the radiant voice, with **Keaton Dunsford** as coauthor
**Status:** Living guide — last touched `20260714.081500` (macOS Step 8 gains `--harden-home`: real credential-store reads denied, dedicated jail-local keys generated outside the jail, and an honest limit named on self-testing from inside an already-jailed window)
...
# then tell the agent inside the jail to wire git config to the new files
```

This mirrors **8b**–**8c** exactly, just generated by your own hands rather than copied from an already-shared real key: a dedicated SSH deploy key per forge and a passphrase-free, signing-only GPG key, all living under this project's own gitignored `.ssh/` and `.gnupg-rye/`. Once both are registered, the agent inside the jail rewires `core.sshCommand` and `gpg.program` to point at these files exclusively (same shape as **8b**/**8c**'s snippets, new paths), and the next `--harden-home` launch can no longer read your real identity at all, read or write.
